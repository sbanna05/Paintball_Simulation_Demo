...
public class PaintballAgent : Agent
{
    ...
    // raycastDirections ELT�VOL�TVA - Ray Perception Sensor haszn�lja
    [SerializeField] private LayerMask obstacleLayerMask;
    [SerializeField] private LayerMask coverLayerMask;

    // MEGJEGYZ�S: Ray Perception Sensor automatikusan hozz�adja:
    ...
    [SerializeField] private Collider nearMissCollider; // A nagyobb collider a k�zelihal�l�rz�kel�shez
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(transform.localPosition);   // 3 (Lok�lis koordin�ta!)
        ...
        sensor.AddObservation(rb.velocity / 10f); // 3 (Sebess�g seg�t a mozg�s tanul�s�ban)
    }
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        if (mouseX < -0.1f)
            discreteActions[1] = 0; // Balra forg�s
        else if (mouseX > 0.1f)
            discreteActions[1] = 2; // Jobbra forg�s
        else
            discreteActions[1] = 1; // Nincs forg�s
